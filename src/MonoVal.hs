...
-- instance (Applicative m, Monad m, MonadState (MapSig Val) m) => MonadEval Val Env m where
-- instance MonadEval Val Env EvalM where
instance (Applicative m, Monad m, Signature Val sig, MonadState sig m) => MonadEval Val Env m where
