...
Concerning substitution lists, we talk about "lists of values" for simplicity. More specifically, we want them to be lists of pointers to avoid the duplication of "real" values during constructing lists like $\multiinsert {\vec v} {\vec k} {w}$. They can not only be implemented as linked lists but also as balanced binary trees, cf. Section~\ref{sec:experiments}.
...
\]
%\end{defin}
First, it is important to notice that $\parseb M \Gamma u {\vec x}$ implies that each variable occurring in $\vec x$ is contained in $\Gamma$ and each free variable occurring in $u$ is not contained in $\Gamma$. This can be shown by induction on $M$ (simultaneously for all sets $\Gamma$). \\
By the same argument, one can see that (for each $M$ and $\Gamma$) there exists a unique tuple $(u, \vec x)$ satisfying $\parseb M \Gamma u  {\vec x}$, so we can consider $\parseFun \Gamma$ a function $\te \rightarrow \otw \times \gl$. Furthermore, we notice that $\parseb M \Gamma u {\vec x}$ always implies $\freevars u = \length {\vec x}$. \\
...
For all $t \in \otc, M \in \te$, we have $\pr t \emptyVec = M$ if and only if $\parseb M \emptySet t \emptyVec$ (which just means $\parseFunE t = M$) as both judgements are defined identical in the case of closed ordered terms. This essentially (with implicit use of the bijection $\otc \leftrightarrow \ot \otimes \vec \emptySet$) means $\prE \circ \parseFunE  = id_{\te}$, i.e. the composition of parsing and printing is the identity.
